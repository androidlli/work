...
import com.cango.palmcartreasure.document.EditActivity;
import com.cango.palmcartreasure.document.EditFragment;
...
public class TaskDetailFragment extends BaseFragment implements TaskDetailContract.View, EasyPermissions.PermissionCallbacks {
    //任务详情中点击分配按钮
    public static final int REQUEST_TASKARRANGE = 10001;
    ...
    private SubActionButton subActionButton1, subActionButton2, subActionButton3, subActionButton4, subActionButton5;
    private FloatingActionMenu rightLowerMenu;

    @OnClick({R.id.tv_download, R.id.ll_callrecord, R.id.ll_home_visit, R.id.ll_trailer_info, R.id.ll_case_info, R.id.ll_customer_info})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_callrecord:
                ...
                iv5.setImageResource(R.drawable.trailer_information_off);
                isTrailerInfomation = false;
                onRefresh(0);
                break;
    ...
    //
    private boolean canSendCar;
    private String mCurrentPhotoPath;
    /**
     * 当前显示的是否是拖车信息
     */
    private boolean isTrailerInfomation;
    private TypeTaskData.DataBean.TaskListBean mTaskListBean;
    /**
     * 0:员工进入；1：老板进入有分配；2：老板进入（抽回进入）
     */
...
//                    Logger.d(dateString + ": Lat = " + aMapLocation.getLatitude() + "   Lon = " + aMapLocation.getLongitude() + "   address = " + aMapLocation.getAddress());
    ...
    //拖车大点成功刷新，并且没有打点按钮,只有员工可以拖车大点
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onCheckPointOkToDetailEvent(CheckPointOkToDetailEvent event) {
        //业务要求可以不可以，拖车大点成功后手动设置，可以进行送车入库
        mTaskListBean.setIsDone("T");
        mTaskListBean.setIsCheckPoint("F");
        if (rightLowerMenu != null) {
            ArrayList<FloatingActionMenu.Item> subActionItems = rightLowerMenu.getSubActionItems();
            //表示是已完成任务不需要显示星际菜单
            if ("F".equals(mTaskListBean.getIsStart()) && "F".equals(mTaskListBean.getIsCheckPoint()) && "F".equals(mTaskListBean.getIsDone())) {

            } else {
                if (isStart) {
                    //原本是5个，那么既然他原本是可以开始任务的，那么他能大点成功的话说明已经开始任务了，并且开始任务后，开始任务按钮一定不在了
                    if (subActionItems.size() > 1) {
                        subActionItems.remove(1);
                        if (subActionButton3 != null) {
                            subActionButton3.setVisibility(View.GONE);
                        }
    ...
    public static TaskDetailFragment newInstance(int type, TypeTaskData.DataBean.TaskListBean taskListBean, int FromType) {
        TaskDetailFragment fragment = new TaskDetailFragment();
        Bundle args = new Bundle();
        args.putInt(TYPE, type);
        args.putParcelable(TASKLISTBEAN, taskListBean);
    ...
    protected void initView() {
        ...
        canCheckPoint = "T".equals(mTaskListBean.getIsCheckPoint());
        canSendCar = "T".equals(mTaskListBean.getIsDone());
        ...
        ImageView rlIcon5 = new ImageView(mActivity);
...
//                        .setEndAngle(270 - 20)
                ...
                subActionButton2.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        //任务分配
                        List<TaskManageList.DataBean.TaskListBean> taskListBeanList = new ArrayList<TaskManageList.DataBean.TaskListBean>();
                        TaskManageList.DataBean.TaskListBean bean = new TaskManageList.DataBean.TaskListBean();
                        bean.setAgencyID(mTaskListBean.getAgencyID());
                        bean.setCaseID(mTaskListBean.getCaseID());
                        bean.setApplyID(mTaskListBean.getApplyID());
                        bean.setApplyCD(mTaskListBean.getApplyCD());
                        bean.setCustomerName(mTaskListBean.getCustomerName());
                        bean.setCarPlateNO(mTaskListBean.getCarPlateNO());
                        bean.setShortName(mTaskListBean.getShortName());
                        bean.setDistance(mTaskListBean.getDistance());
                        bean.setAgencyStatus(mTaskListBean.getAgencyStatus());
                        bean.setFlowStauts(mTaskListBean.getFlowStauts());
                        bean.setFeerate("");
                        bean.setRedueDays(0);
                        bean.setRedueAmount("");
                        ...
                        if (!CommUtil.checkIsNull(taskListBeanList) && taskListBeanList.size() > 0) {
                            ...
                            startActivityForResult(intent, REQUEST_TASKARRANGE);
//                            mActivity.mSwipeBackHelper.forward(intent, REQUEST_TASKARRANGE);
                        }
                    }
                });
                ...
                subActionButton4.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        Intent trailNavIntent = new Intent(mActivity, EditActivity.class);
                        trailNavIntent.putExtra(EditFragment.BEAN, mTaskListBean);
                        mActivity.mSwipeBackHelper.forward(trailNavIntent);
                    }
                });
...
//                        .setEndAngle(270 - 20)
            ...
            //写标注
            rlIcon5.setImageDrawable(getResources().getDrawable(R.drawable.biji));
...
//                            openCamera();
            ...
            subActionButton5 = rLSubBuilder.setContentView(rlIcon5).setLayoutParams(subParams).build();
            subActionButton5.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent trailNavIntent = new Intent(mActivity, EditActivity.class);
                    trailNavIntent.putExtra(EditFragment.BEAN, mTaskListBean);
...
//                    rightLowerButton.setAlpha(0.3f);
    ...
    protected void initData() {
        ...
        if (getArguments() != null) {
            ...
            if (getArguments().containsKey("FromType")) {
                mFromType = getArguments().getInt("FromType");
            }
        }
        initLocation();
    }
                                    ...
                                    protected void _onNext(TaskAbandon o) {
                                        if (isAdded()) {
                                            ...
                                            if (code == 0) {
                                                ...
                                                //开始任务成功后刷新星际菜单
                                                if (isStart) {
                                                    //原本是5个，那么既然他原本是可以开始任务的
                                                    ArrayList<FloatingActionMenu.Item> subActionItems = rightLowerMenu.getSubActionItems();
                                                    if (subActionItems.size() > 0) {
                                                        subActionItems.remove(0);
                                                        if (subActionButton1 != null) {
                                                            subActionButton1.setVisibility(View.GONE);
                                                        }
                                                        rightLowerMenu.updateItemPositions();
                                                    }
                                                }
...
//            showDotDialog();
        ...
        //管理员点击分配按钮,并且成功分配了,那么发送通知到未分配列表界面刷新数据
        if (requestCode == REQUEST_TASKARRANGE && resultCode == Activity.RESULT_OK) {
            if (rightLowerMenu != null) {
                ArrayList<FloatingActionMenu.Item> subActionItems = rightLowerMenu.getSubActionItems();
                if (subActionItems.size() > 2) {
                    subActionItems.remove(1);
                    if (subActionButton2 != null) {
                        subActionButton2.setVisibility(View.GONE);
                    }
                    rightLowerMenu.updateItemPositions();
                    EventBus.getDefault().post(new AllotTaskDetailToAdminTaskEvent(""));
                }
            }
        }
    }
...
//            itemHolder.tvContent.setTextColor(getResources().getColor(taskInfo.getRightColor()));
            ...
            if ("客户信息".equals(title)||"紧急联系人信息".equals(title)) {
                itemHolder.tvContent.setVisibility(View.GONE);
                itemHolder.tvContent1.setVisibility(View.VISIBLE);
                itemHolder.tvContent1.setText(taskInfo.getRight());
                itemHolder.tvContent1.setTextColor(getResources().getColor(taskInfo.getRightColor()));
            } else {
                itemHolder.tvContent.setVisibility(View.VISIBLE);
                itemHolder.tvContent1.setVisibility(View.GONE);
                itemHolder.tvContent.setText(taskInfo.getRight());
                itemHolder.tvContent.setTextColor(getResources().getColor(taskInfo.getRightColor()));
            }
    ...
    class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        public ItemViewHolder(View view) {
            ...
            tvContent1 = (TextView) rootView.findViewById(R.id.tv_task_detail_content1);
        }
    }
